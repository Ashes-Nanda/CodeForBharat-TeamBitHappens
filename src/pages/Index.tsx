...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const shouldShowChatbot = localStorage.getItem('showAshaChatbot') === 'true';
    if (shouldShowChatbot) {
      // Add a small delay to ensure the dashboard is fully loaded
      setTimeout(() => {
        setShowChatbot(true);
        // Clear the flag after showing
        localStorage.removeItem('showAshaChatbot');
      }, 500);
    }
  }, []);

  // Force chatbot to open if it's the first visit
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <AshaChatbot 
        initialOpen={showChatbot} 
        key={showChatbot ? 'open' : 'closed'} // Force re-render when showChatbot changes
      />
      <EmergencyContacts />
    </motion.div>
  );
